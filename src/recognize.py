    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 5) -> List[FaceDet]:
        ...
        if faces.shape[0] > 0:
            ...
            for (x, y, w, h) in faces:
                ...
                rx1, ry1, rx2, ry2 = _clip_xyxy(cx - side / 2.0, cy - side / 2.0, cx + side / 2.0, cy + side / 2.0, W, H)
        ...
        # Full-frame fallback if no faces were detected by Haar + ROI FaceMesh
        if len(out) == 0:
            kps_full = self._full_facemesh_5pt(frame_bgr)
            if kps_full is not None and _kps_span_ok(kps_full, min_eye_dist=10.0):
                bb = _bbox_from_5pt(kps_full, pad_x=0.55, pad_y_top=0.85, pad_y_bot=1.15)
                x1, y1, x2, y2 = _clip_xyxy(bb[0], bb[1], bb[2], bb[3], W, H)
                out.append(
                    FaceDet(
                        x1=x1, y1=y1, x2=x2, y2=y2,
                        score=1.0,
                        kps=kps_full.astype(np.float32),
                    )
                )
