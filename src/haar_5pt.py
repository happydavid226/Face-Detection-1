...
def _estimate_norm_5pt(kps_5x2: np.ndarray, out_size: Tuple[int, int] = (112, 112)) -> np.ndarray:
    ...
    M, _ = cv2.estimateAffinePartial2D(k, dst, method=cv2.RANSAC)
    if M is None:
        M, _ = cv2.estimateAffine2D(k, dst)
    if M is None:
...
class Haar5ptDetector:
    ...
    def _full_facemesh_5pt(self, frame_bgr: np.ndarray) -> Optional[np.ndarray]:
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 1) -> List[FaceKpsBox]:
        ...
        if faces.shape[0] > 0:
            ...
            rx1 = max(0, int(cx - side / 2.0))
            ry1 = max(0, int(cy - side / 2.0))
            rx2 = min(W, int(cx + side / 2.0))
            ry2 = min(H, int(cy + side / 2.0))
        ...
        # Fallback to MediaPipe full frame if Haar or ROI FaceMesh returned None
        if kps is None:
            kps = self._full_facemesh_5pt(frame_bgr)
            if kps is None:
                if self.debug:
                    print("[haar_5pt] No face detected by Haar or MediaPipe FaceMesh")
                return []
